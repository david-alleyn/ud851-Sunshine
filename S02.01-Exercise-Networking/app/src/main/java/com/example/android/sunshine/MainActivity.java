...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadWeatherData() {
        ...
        String preferedLocationQuery = SunshinePreferences.getPreferredWeatherLocation(context);
        URL locationQueryURL = NetworkUtils.buildUrl(preferedLocationQuery);
        new DataGatherTask().execute(locationQueryURL);
    }

    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    public class DataGatherTask extends AsyncTask<URL, Void, String> {

        @Override
        protected String doInBackground(URL... params) {
            URL url = params[0];
            String localQueryResults = null;
            try {
                localQueryResults = NetworkUtils.getResponseFromHttpUrl(url);
            } catch (IOException e) {
                e.printStackTrace();
            }
